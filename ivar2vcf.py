...
def strand_bias_filter(format,file_out):
    """
    Description:
    ...
    # o = ("^^^or = "+str(oddsr)+" pval = " + str(pvalue) + ", "+str(format[0])+"\n")
    # fout.write(o)
    # fout.close()
...
def write_vcf_header(ref, ignore_strand_bias, file_out, filename):
    ...
    if not ignore_strand_bias:
        ...
        header_filter += [
            '##FILTER=<ID=sb,Description="Strand-bias fisher-test p-value < 0.01">'
        ]
        ...
        for line in fin:
            if not re.match("REGION", line):
                ...
                if not args.ignore_strand_bias:
                    if strand_bias_filter(format,args.file_out):
                        if filter:
                            filter += "," + strand_bias_filter(format,args.file_out)
                        else:
                            filter = strand_bias_filter(format,args.file_out)
